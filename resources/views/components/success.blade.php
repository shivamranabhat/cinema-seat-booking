@if(session()->has('success'))
    <div class="flash fixed bg-white bottom-2 right-2 z-10 rounded-md" x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 2000)"
    ...
    <div class="success flex gap-2 p-4">
        ...
        <div class="message">
            <div class="title">
                <h5 class="text-base md:text-[1.125rem] lg:text-[1.125rem] xl:text-[1.125rem] font-bold">{{ session('success') }}</h5>
            </div>
            <div class="description">
                <p class="text-sm text-brand-600 font-medium">Successfull!</p>
            </div>
        </div>
    </div>
</div>
